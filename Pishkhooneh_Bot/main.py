...
def crawler():
    while 1:
        ...
        for item in totalData:
            if item.get('totalPrice') == None:
                newItem=item
                newItem['totalPrice']=0
                correctedPrice.append(newItem)
            else:
                correctedPrice.append(item)
        correctedPrice.reverse()
        data=correctedPrice.copy()
        correctedPrice.clear()
            ...
            with sqlite3.connect('Pishkhooneh_Bot/db.sqlite3') as connection:
                try:
                        ...
                        for item in data:
                            ...
                            cur.execute('SELECT * FROM estimate_homes2 WHERE id=%s'%item.get('identifier'))
                            if not cur.fetchone():
