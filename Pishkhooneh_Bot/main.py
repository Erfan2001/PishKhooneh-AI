        ...
        with sqlite3.connect('Pishkhooneh_Bot/db.sqlite3') as connection:
            try:
                    for item in data:
                        ...
                        ownerId=0
                        if item.get('contact'):
                            if item.get('contact').get('type')=='AGENCY':
                                ownerId=item.get('contact').get('departmentId')
                        print('Number => ',data.index(item),' | Identifier:',id1)
                        cur.execute(f'INSERT INTO estimate_home VALUES ({id1},"{title}",{totalPrice},{unitPrice},"{province}","{address}",{locationX},{locationY},"{main_image}","{image1}","{image2}","{image3}","{type1}","{category}","{status}","{seller}",{age},{floorArea},{numOfBeds},{parking},{lobby},{wareHouse},{sportHall},{guard},{elevator},{swimmingPool},{balcony},{roofGarden},{remoteDoor},"{description}",{region},"{neighbor}",NULL,"{phoneNumber}","{date}","{date}",{isActive},{ownerId})')
                        connection.commit()
